...
function Trajectory() {
  ...
  const [showFuture, setShowFuture] = useState(false); // IPCC 2050 overlay

  const series = window.CO2_ANNUAL; // 47 points
  const monthly = window.CO2_SERIES; // 564 points
  const ssp = window.SSP_SCENARIOS;

  const W = 760, H = 360;
  const padL = 56, padR = 24, padT = 24, padB = 36;
  const yMin = 332;
  const yMax = showFuture ? 620 : 432;
  const xMaxYear = showFuture ? 2050 : 2025;
  const yearSpan = xMaxYear - 1979;
  const sx = (year) => padL + ((year - 1979) / yearSpan) * (W - padL - padR);
  ...
  // IPCC scenarios path (from 2025 to 2050)
  const sspPaths = useMemo(() => {
    if (!showFuture) return null;
    const toPath = (data) => data.map((d, i) => `${i === 0 ? 'M' : 'L'}${sx(d.year).toFixed(1)},${sy(d.value).toFixed(1)}`).join(' ');
    return {
      'SSP1-2.6': toPath(ssp['SSP1-2.6'].data),
      'SSP5-8.5': toPath(ssp['SSP5-8.5'].data),
    };
  }, [showFuture]);
  ...
  const onSvgMove = (e) => {
    ...
    const year = 1979 + ((realX - padL) / (W - padL - padR)) * yearSpan;
  ...
  // Adaptive ticks
  const xTicks = showFuture
    ? [1980, 1990, 2000, 2010, 2020, 2030, 2040, 2050]
    : [1980, 1990, 2000, 2010, 2020, 2025];
  const yTicks = showFuture
  ...
  return (
    <section ref={ref} className="section" id="sec-2" data-screen-label="02 Trajectory">
      ...
      <div style={{ position: 'relative', zIndex: 2, display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 24, alignItems: 'start' }}>
        {/* Left: chart */}
        <div className="glass" data-presenter="trajectory-chart" style={{ padding: 24, position: 'relative' }}>
          <div className="hstack" style={{ justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 12 }}>
            <div>
              ...
              <div style={{ fontSize: 15, color: 'var(--text-dim)' }}>
                {showFuture
                  ? '1979 → 2050 · observé + projections IPCC AR6'
                  : '1979 → 2025 · données mensuelles, série annuelle moyennée'}
              </div>
            </div>
            <div className="hstack" style={{ gap: 6, flexWrap: 'wrap' }}>
              ...
              <button className={`btn-glass ${showFuture ? 'active' : ''}`}
                      onClick={() => setShowFuture(s => !s)}
                      style={{ marginLeft: 8, borderColor: showFuture ? 'rgba(82,255,184,0.4)' : undefined }}>
                {showFuture ? 'Masquer 2050' : 'Voir 2050 (IPCC)'}
              </button>
              <ExportButton targetSelector="#sec-2 .glass:nth-of-type(1)"
                            filename="trajectoire_co2"
                            style={{ marginLeft: 4 }} />
            </div>
          </div>
          ...
          <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}
            ...
            {/* "Today" vertical separator when showing future */}
            {showFuture && (
              <g>
                <line x1={sx(2025)} x2={sx(2025)} y1={padT} y2={H - padB}
                  stroke="rgba(255,255,255,0.20)" strokeDasharray="3 3" />
                <text x={sx(2025) + 4} y={padT + 12}
                  fontSize="9" fontFamily="JetBrains Mono"
                  fill="rgba(255,255,255,0.55)" textTransform="uppercase" letterSpacing="0.1em">
                  aujourd'hui
                </text>
              </g>
            )}
            ...
            {/* IPCC scenarios — drawn before the observed curve so it stays on top */}
            {showFuture && sspPaths && (
              <g>
                <path d={sspPaths['SSP1-2.6']} stroke={ssp['SSP1-2.6'].color}
                  strokeWidth="2" strokeDasharray="4 3" fill="none" opacity="0.85" />
                <path d={sspPaths['SSP5-8.5']} stroke={ssp['SSP5-8.5'].color}
                  strokeWidth="2" strokeDasharray="4 3" fill="none" opacity="0.85" />
                {/* Labels at 2050 */}
                <text x={sx(2050) - 4} y={sy(441) - 6} textAnchor="end"
                  fontSize="10" fontFamily="JetBrains Mono" fill={ssp['SSP1-2.6'].color} fontWeight="600">
                  SSP1-2.6 · 441 ppm
                </text>
                <text x={sx(2050) - 4} y={sy(607) + 14} textAnchor="end"
                  fontSize="10" fontFamily="JetBrains Mono" fill={ssp['SSP5-8.5'].color} fontWeight="600">
                  SSP5-8.5 · 607 ppm
                </text>
              </g>
            )}
            ...
            {/* Events */}
            {events.map((ev, i) => {
              const isPicked = pickedEvent && pickedEvent.name === ev.name && Math.abs(pickedEvent.year - ev.year) < 0.01;
              return (
                <g key={i} onClick={() => setPickedEvent(isPicked ? null : ev)} style={{ cursor: 'pointer' }}>
                  <line x1={sx(ev.year)} x2={sx(ev.year)} y1={padT} y2={H - padB}
                    stroke={ev.color} strokeOpacity={isPicked ? 0.55 : 0.18} strokeDasharray="2 3"
                    strokeWidth={isPicked ? 1.5 : 1} />
                  <circle cx={sx(ev.year)} cy={H - padB + 14} r={isPicked ? 5 : 3.5}
                    fill={ev.color}
                    style={{ filter: isPicked ? `drop-shadow(0 0 6px ${ev.color})` : 'none' }} />
                </g>
              );
            })}
          ...
          {/* Legend */}
          <div className="hstack" style={{ marginTop: 12, gap: 18, fontSize: 14, color: 'var(--text-dim)', flexWrap: 'wrap' }}>
            ...
            <span className="hstack" style={{ gap: 6 }}>
              ...
              <GlossaryTerm term="IC95">IC 95%</GlossaryTerm> <GlossaryTerm term="Sen">Sen</GlossaryTerm>
            </span>
            <span className="hstack" style={{ gap: 6 }}>
            ...
            {showFuture && (
              <>
                <span className="hstack" style={{ gap: 6 }}>
                  <span style={{ width: 14, height: 0, borderTop: '2px dashed #52FFB8' }}></span>
                  SSP1-2.6
                </span>
                <span className="hstack" style={{ gap: 6 }}>
                  <span style={{ width: 14, height: 0, borderTop: '2px dashed #FF6B35' }}></span>
                  SSP5-8.5
                </span>
              </>
            )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <MetricCard
            ...
            accent="var(--hot2)"
          />
          <div className="glass" data-presenter="trajectory-vostok" style={{ padding: 22 }}>
            ...
            {/* Speed comparison — paléo vs anthropique */}
            <div style={{
              ...
              <div className="mono" style={{ fontSize: 10, color: 'var(--text-dim)', letterSpacing: '0.16em', textTransform: 'uppercase', marginBottom: 10 }}>
                Vitesse de variation
              </div>
              <svg viewBox="0 0 280 84" style={{ width: '100%', display: 'block' }}>
                {/* Two slopes on the same y-span (40 px), different x-spans
                    representing 100 ppm change in 10 000 yrs (paléo) vs 50 yrs (anthropique). */}
                {/* Paléo slope: gentle */}
                <g>
                  <text x={10} y={14} fontSize="9" fontFamily="JetBrains Mono" fill="#52FFB8" letterSpacing="0.08em">PALÉO</text>
                  <line x1={10} y1={70} x2={130} y2={68} stroke="#52FFB8" strokeWidth="2" />
                  <circle cx={10} cy={70} r="2.4" fill="#52FFB8" />
                  <circle cx={130} cy={68} r="2.4" fill="#52FFB8" />
                  <text x={10} y={48} fontSize="13" fontFamily="Space Grotesk" fill="#52FFB8" fontWeight="600">~0.01</text>
                  <text x={50} y={48} fontSize="10" fontFamily="JetBrains Mono" fill="#9CA3AF">ppm/an</text>
                  <text x={10} y={82} fontSize="8" fontFamily="JetBrains Mono" fill="#9CA3AF" opacity="0.7">~100 ppm sur 10 000 ans</text>
                </g>
                {/* Vertical separator */}
                <line x1={146} y1={8} x2={146} y2={82} stroke="rgba(255,255,255,0.08)" strokeDasharray="2 2" />
                {/* Anthropique slope: steep */}
                <g>
                  <text x={158} y={14} fontSize="9" fontFamily="JetBrains Mono" fill="#FF6B35" letterSpacing="0.08em">AUJOURD'HUI</text>
                  <line x1={158} y1={70} x2={262} y2={26} stroke="#FF6B35" strokeWidth="2.5" />
                  <circle cx={158} cy={70} r="2.4" fill="#FF6B35" />
                  <circle cx={262} cy={26} r="3" fill="#FF6B35" />
                  <text x={158} y={48} fontSize="13" fontFamily="Space Grotesk" fill="#FF6B35" fontWeight="600">+1.88</text>
                  <text x={198} y={48} fontSize="10" fontFamily="JetBrains Mono" fill="#9CA3AF">ppm/an</text>
                  <text x={158} y={82} fontSize="8" fontFamily="JetBrains Mono" fill="#9CA3AF" opacity="0.7">+91 ppm sur 47 ans</text>
                </g>
              </svg>
              <div className="mono" style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 6, textAlign: 'center' }}>
                Rapport · <span style={{ color: 'var(--green)', fontWeight: 600 }}>×100 à ×200</span> plus rapide
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Timeline events */}
      <div data-presenter="trajectory-events" style={{ marginTop: 40, position: 'relative', zIndex: 2 }}>
            ...
            {events.map((ev, i) => {
              ...
              return (
                <button key={i}
                  ...
                  title={`${ev.name} (${Math.floor(ev.year)}) — ${ev.impact || ev.desc}`}
                />
              );
            })}
          </div>
          {pickedEvent && (
            <div style={{ marginTop: 18, padding: '18px 22px', background: 'rgba(255,255,255,0.03)', borderRadius: 10, border: `1px solid ${pickedEvent.color}33` }}>
              <div className="hstack" style={{ gap: 12, alignItems: 'baseline', marginBottom: 8 }}>
                <span className="display" style={{ fontSize: 24, color: pickedEvent.color }}>{pickedEvent.name}</span>
                <span className="mono" style={{ fontSize: 14, color: 'var(--text-dim)' }}>· {Math.floor(pickedEvent.year)}</span>
              </div>
              <div style={{ fontSize: 15, color: 'var(--text)', marginBottom: 12 }}>{pickedEvent.desc}</div>
              {pickedEvent.impact && (
                <div style={{
                  display: 'inline-flex', alignItems: 'center', gap: 8,
                  padding: '6px 12px', borderRadius: 6,
                  background: `${pickedEvent.color}18`,
                  border: `1px solid ${pickedEvent.color}55`,
                  marginBottom: 10,
                }}>
                  <span className="mono" style={{ fontSize: 10, color: pickedEvent.color, letterSpacing: '0.14em', textTransform: 'uppercase' }}>Impact mesuré</span>
                  <span className="display tabular" style={{ fontSize: 16, color: pickedEvent.color, fontWeight: 600 }}>{pickedEvent.impact}</span>
                </div>
              )}
              {pickedEvent.impactDetail && (
                <div style={{ fontSize: 14, color: 'var(--text-dim)', lineHeight: 1.55, marginBottom: 8 }}>{pickedEvent.impactDetail}</div>
              )}
              {pickedEvent.source && (
                <div className="mono" style={{ fontSize: 11, color: 'var(--text-dim)', opacity: 0.7, marginTop: 6 }}>↘ {pickedEvent.source}</div>
              )}
            </div>
          )}
          {!pickedEvent && (
            <div style={{ marginTop: 14, fontSize: 14, color: 'var(--text-dim)' }}>
              Cliquez un marqueur pour voir l'impact mesuré sur la trajectoire CO₂.
            </div>
          )}
        </div>
      </div>
